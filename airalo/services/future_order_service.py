...
class FutureOrderService:
    def __init__(self, config: Config, http_resource: HttpResource, signature: Signature, access_token: str):
        if not access_token:
    ...
    def create_future_order(self, payload: dict) -> dict:
        ...
        if self.http.code != 200:
            raise AiraloException(f"Future order creation failed, status code: {self.http.code}, response: {response}")
    ...
    def cancel_future_order(self, payload: dict) -> dict:
        ...
        if self.http.code != 200:
            raise AiraloException(f"Future order cancellation failed, status code: {self.http.code}, response: {response}")
    ...
    def _validate_future_order(self, payload: dict) -> None:
        if not payload.get('package_id'):
            raise AiraloException(f'The package_id is required, payload: {json.dumps(payload)}')
        ...
        if payload['quantity'] > SdkConstants.FUTURE_ORDER_LIMIT:
            raise AiraloException(f'The packages count may not be greater than {SdkConstants.BULK_ORDER_LIMIT}')

        due_date = payload.get('due_date')
        if not due_date:
            raise AiraloException(f'The due_date is required (format: Y-m-d H:i), payload: {json.dumps(payload)}')
        ...
        try:
            ...
            raise AiraloException(f'The due_date must be in the format Y-m-d H:i, payload: {json.dumps(payload)}')

    def _validate_cancel_future_order(self, payload: dict) -> None:
        if not isinstance(payload.get('request_ids'), list) or not payload['request_ids']:
            raise AiraloException(f'The request_ids is required, payload: {json.dumps(payload)}')
